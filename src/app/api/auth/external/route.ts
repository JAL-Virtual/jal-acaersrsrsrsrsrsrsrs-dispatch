...
const retryRequest = async <T>(requestFn: () => Promise<T>, attempts: number = EXTERNAL_API_CONFIG.retryAttempts): Promise<T> => {
  try {
    return await requestFn();
  } catch (error: unknown) {
    const err = error as { code?: string };
    if (attempts > 1 && (err.code === 'ECONNABORTED' || err.code === 'ENOTFOUND' || err.code === 'ECONNREFUSED')) {
      console.log(`Retrying external API request, ${attempts - 1} attempts remaining...`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const apiUrl = externalApiUrl || EXTERNAL_API_CONFIG.baseUrl;
